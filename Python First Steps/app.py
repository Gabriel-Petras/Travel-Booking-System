...
class Flight(db.Model):
    ...
    from_city = db.Column(db.String(100), nullable=False)               # Origin city
    to_city = db.Column(db.String(100), nullable=False)                 # Destination city
    departure_date = db.Column(db.Date, nullable=False)                 # Departure date
    return_date = db.Column(db.Date, nullable=False)                    # Return date
    available_seats = db.Column(db.Integer, nullable=False)             # Available seats
    booked_seats = db.Column(db.Integer, nullable=False, default=0)     # Booked seats
    price = db.Column(db.Float, nullable=False)                         # Price per ticket
    flight_number = db.Column(db.String(50), nullable=True)             # Optional, flight number
    flight_time = db.Column(db.String(50), nullable=True)               # Optional, departure time
...
def book_flight(flight_id):
    ...
    if flight.available_seats <= 0:
        return "Sorry, no available seats for this flight.", 400
...
def search():
    ...
    # Use the current year if no year is provided in the input date range
    current_year = datetime.now().year

    if len(date_range) == 2:
        try:
            # Add current year to the parsed date to ensure correct comparison
            ...
            # If the start date is later than the current date, adjust it to the previous year
    ...
    if start_date and end_date:
        ...
        # Modify the query to filter the departure and return dates properly
        query = query.filter(
    ...
    if dates:  # This checks if the dates query parameter exists
        search_message.append(f"for dates {dates}")
    if guests:
        search_message.append(f"for {guests} guest(s)")
    
    # Join the search_message parts with spaces
...
def cancel_booking(booking_id):
    ...
    if request.method == 'POST':
        ...
        return redirect(url_for('profile', user_id=booking.user_id))
